...
**Goal:** Move the superpowers skill bundle from the external HIVE workspace into the GearCore repository, ship it with the package, and add an `update-superpowers` command with lightweight upstream drift detection.
...
- [ ] **Step 1: Copy skill bundles from HIVE**

Run:

```bash
mkdir -p third_party/superpowers/skills
cp -r /home/r345/workspace/HIVE/third_party/superpowers/skills/* third_party/superpowers/skills/
```
...
```bash
cd /home/r345/workspace/HIVE/third_party/superpowers
git rev-parse HEAD
```
...
**Interfaces:**
- Produces: Old HIVE symlinks removed; GearCore uses bundled copy.

- [ ] **Step 1: Remove stale HIVE symlinks**
...
for name in brainstorming dispatching-parallel-agents executing-plans finishing-a-development-branch receiving-code-review requesting-code-review subagent-driven-development systematic-debugging test-driven-development using-git-worktrees using-superpowers verification-before-completion writing-plans writing-skills; do
  if [ -L "$name" ] && readlink "$name" | grep -q "/HIVE/"; then
    rm "$name"
  fi
done
```
