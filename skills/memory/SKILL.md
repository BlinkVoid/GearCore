# Skill: Persistent Memory (MemCore)

## Overview
Access to HIVE's persistent memory system for querying historical context, past decisions, and learned knowledge.
...
```bash
gearcore call memcore mem_query '{"query": "What was the auth approach for project X?", "quadrant_hint": "coding"}'
```

### mem_save
Store new knowledge to short-term buffer for future reference.
```bash
gearcore call memcore mem_save '{"content": "...", "summary": "...", "quadrants": ["coding"]}'
```

### add_task
Add a task to the memory system.
```bash
gearcore call memcore add_task '{"title": "Review auth middleware", "priority": "medium"}'
```
