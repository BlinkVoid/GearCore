...
```bash
gearcore call hive-gateway worker_register '{"name": "worker-1", "capabilities": ["code", "research"]}'
```
...
```bash
gearcore call hive-gateway worker_poll_task '{"worker_id": "..."}'
```

### worker_heartbeat
Report progress mid-task (for long tasks). If response contains `should_stop: true`, submit partial results and shut down.
```bash
gearcore call hive-gateway worker_heartbeat '{"worker_id": "...", "status": "working"}'
```

### worker_submit_result
Submit your completed work.
```bash
gearcore call hive-gateway worker_submit_result '{"worker_id": "...", "task_id": "...", "result": "..."}'
```

### worker_get_messages
Check for operator commands each poll cycle.
```bash
gearcore call hive-gateway worker_get_messages '{"worker_id": "..."}'
```

### worker_deregister
Leave the pool when done.
```bash
gearcore call hive-gateway worker_deregister '{"worker_id": "..."}'
```
